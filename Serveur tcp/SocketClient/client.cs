...
// L'opérateur await permet d'attendre la connexion asyncrone* du client vers le serveur. Lorsque le client est connecté, await n'attend plus de connexion et passe au reste du program.
// *La connexion asyncrone est un type de connexion "différé".

while (true) // Tant que la connexion est valide
{
    ...
    // Envoie le message et attend que le serveur est reçu le message.
    ...
    if(reponse == "Hello Client !") // Si le message envoyé par le serveur est identique.
    {
...
client.Shutdown(SocketShutdown.Both); // Désactive la connexion du client (Both pour arrêter l'envoi et la réception de donnée)